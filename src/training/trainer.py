...
class Trainer:
    ...
    def train_one_epoch(self) -> float:
        ...
        for images, hr_images, targets, target_lengths, _, _ in pbar:
            ...
            # Forward under AMP (model forward + optional SR)
            ...
            with autocast('cuda'):
                ...
                if use_sr and (sr_out is not None):
                    ...
                    # If STN grid exists, warp HR target to match SR output
                    if grid is not None:
                        hr_target = torch.nn.functional.grid_sample(hr_target, grid, align_corners=False)

                    sr_loss = self.sr_criterion(sr_out, hr_target)

            # CTC loss in FP32 (stable)
            preds_permuted = preds.float().permute(1, 0, 2)  # [T, B, C]
            ...
            if use_sr and (sr_loss is not None):
                loss = loss + sr_loss.float() * 1.0  # SR Weight = 1.0
# Scale loss & backward
